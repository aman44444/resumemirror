...
export default function Home() {
  ...
  return (
    <div
      ...
      style={{
        background:
          "radial-gradient(ellipse at 20% 0%, rgba(180,180,210,0.18) 0%, transparent 55%), radial-gradient(ellipse at 80% 5%, rgba(160,200,185,0.14) 0%, transparent 50%), #F7F7F5",
      }}
    >
      <LiquidGlassSVG />

      <nav className="sticky top-0 z-50 px-6 py-4">
        <div className="max-w-5xl mx-auto flex flex-col items-start gap-2">
          ...
          <span className="text-xs text-gray-400 pl-1">
            AI that tailors your resume like a recruiter
          </span>
        </div>
      </nav>
      <div className="max-w-5xl mx-auto px-6 py-10">
        {isLoading ? (
          ...
          <>
            <div className="text-center mb-10">
              ...
              <p className="text-gray-400 text-base max-w-md mx-auto">
                AI that reads your resume like a recruiter — then tells you
                exactly what to fix
              </p>
            </div>
          ...
          <div className="max-w-3xl mx-auto">
            ...
            {activeTab === "tailor" && tailor.result && (
              <ResultsView result={tailor.result} onReset={resetAll} />
            )}
            {activeTab === "redflags" && redFlags.result && (
              <RedFlagResults data={redFlags.result} />
            )}
            {activeTab === "interview" && interview.result && (
              <InterviewPrep data={interview.result} />
            )}
          </div>
        )}
      </div>
