...
export default function InterviewPrep({ data }: { data: InterviewResult }) {
  ...
  const readinessColor =
    data.overallReadiness >= 70 ? "text-green-600"
    : data.overallReadiness >= 40 ? "text-amber-600"
    : "text-red-600"
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-3">
        ...
        {data.questions.map((q, i) => {
          const config = DIFFICULTY_CONFIG[q.difficulty]
          const isOpen = openIndex === i
          const answerRevealed = revealedAnswers.has(i)
          return (
            <div key={i} className={`bg-white border rounded-xl overflow-hidden transition-all ${isOpen ? config.border : "border-gray-200"}`}>
              <button
